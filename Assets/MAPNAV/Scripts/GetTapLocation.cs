...
public class GetTapLocation : MonoBehaviour
{
    ...
    void Update()
    {
        if (gps.gpsFix && gps.ready)
        {
            ...
            //UNITY EDITOR (MOUSE CLICK)
            #if UNITY_EDITOR
            if (Input.GetMouseButtonDown(0)) {
                touchPos = mainCam.ScreenToWorldPoint(new Vector3(Input.mousePosition.x,Input.mousePosition.y,0));
                Touch2WGS84();
            }
            #endif
        }
    ...
    //Translate screen coordinates into latitude/longitude(WGS84) and draw circle wave effect at touch position
    void Touch2WGS84() {
        posX = touchPos.x;
        posZ = touchPos.z;
