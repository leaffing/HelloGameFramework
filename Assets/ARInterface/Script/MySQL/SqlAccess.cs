...
public class SqlAccess
{
    ...
    public void SendFileBytesToDatabase(string title, byte[] fileArr)
    {
        string sendFileSql = "insert into file(title,file) values(?title,?file);";
        MySqlCommand sendCmd = new MySqlCommand(sendFileSql, dbConnection);
        ...
        try
        {
            ...
            Debug.Log("向数据库储存文件数据完成");
        }
        catch (Exception e)
        {
            Debug.Log("向数据库存储文件数据失败：" + e.Message);
        }
        finally
        {
        }
    ...
    public byte[] GetFileFromDatabase(string searchTitle)
    {
        string getFileSql = "select * from file where title='" + searchTitle + "';";
        MySqlCommand getCmd = new MySqlCommand(getFileSql, dbConnection);
