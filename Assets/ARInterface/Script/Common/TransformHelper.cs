...
namespace Common
{
    ...
    public static class TransformHelper
    {
        ...
        /// <returns></returns>
        public static float MyDistance(this Transform one, Transform two)//计算俩者之间的距离的平方（节省内存）
        {
            Vector3 there = one.position - two.position;
            return ((there.x * there.x) + (there.y * there.y) + (there.z * there.z));
        }
