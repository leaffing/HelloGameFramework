...
#endif
#else
            m_WWW = new WWW(GameFramework.Utility.Path.GetRemotePath(fullPath));
#endif
        }
        ...
        private void UpdateWWW()
        {
            if (m_WWW != null)
            {
                if (m_WWW.isDone)
                {
                    ...
                    {
                        m_LoadResourceAgentHelperErrorEventHandler(this, new LoadResourceAgentHelperErrorEventArgs(LoadResourceStatus.NotExist, GameFramework.Utility.Text.Format("Can not load asset bundle '{0}' with error message '{1}'.", m_BytesFullPath, m_WWW.error)));
                    }
                }
                else
                {
