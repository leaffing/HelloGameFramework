...
namespace Custom.UI
{
    [AddComponentMenu("CustomUI/ImageUI", 11)]
    public class ImageUI : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        public enum Type
        {
            Simple,
        ...
        /// Filling method for filled normalSprites.
        ...
        // Not serialized until we support read-enabled normalSprites better.
        [FormerlySerializedAs("AlphaHitTestMinimumThreshold")]
        [Range(0, 1)]
        [SerializeField]
        public float m_AlphaHitTestMinimumThreshold = 0;
        ...
        private List<Vector2> m_AreaPointVerticalContainner = new List<Vector2>();
        public List<Vector3> m_AreaScreenPointContainner = new List<Vector3>();
        public List<Vector3> AreaScreenPointContainner
        {
            get
            {
                return m_AreaScreenPointContainner;
            }
            set
            {
                m_AreaScreenPointContainner = value;
                SetVerticesDirty();
            }
        }
        private void CreatePolygon(VertexHelper toFill)
        {
            toFill.Clear();
            m_AreaPointVerticalContainner.Clear();
            ...
            var curVertice = Vector2.zero;// m_Parent.anchoredPosition;
            var verticeCount = m_AreaScreenPointContainner.Count;
            for (int i = 0; i < verticeCount; i++)
            {
                curVertice = Extension.GetPositionInLoccalRectangle(rectTransform, m_AreaScreenPointContainner[i]);
                ...
                m_AreaPointVerticalContainner.Add(curVertice);
            }
            int[] triangles = m_AreaPointVerticalContainner.GetTriangulateIndex();
            for (int i = 0, vIdx = 1; i < triangles.Length; i += 3, vIdx++)
            {
        ...
        protected override void OnPopulateMesh(VertexHelper toFill)
        {
            if (activeSprite == null)
            {
                if (m_AreaScreenPointContainner.Count < 3)
                    base.OnPopulateMesh(toFill);
                else
                {
                    var bounds = m_AreaScreenPointContainner.GetBoundRect();
                    rectTransform.position = bounds.center;
                    rectTransform.sizeDelta = bounds.size;
                    CreatePolygon(toFill);
                }
                return;
            }
        ...
        void GenerateFilledSprite(VertexHelper toFill, bool preserveAspect)
        {
            ...
            // Horizontal and vertical filled normalSprites are simple -- just end the Image prematurely
        ...
        public virtual bool IsRaycastLocationValid(Vector2 screenPoint, Camera eventCamera)
        {
            ...
            if (activeSprite == null)
            {
                if ((m_AreaPointVerticalContainner?.Count ?? 0) < 3)
                    return true;
                if (m_AreaPointVerticalContainner.ContainsThePoint(local))
                    return true;
                else
                    return false;
            }
        ...
        public void CustomAnchor()
        {
            var Parent = this.rectTransform.parent as RectTransform;
            var ParentSize = new Vector2(1.0f / Parent.rect.size.x, 1.0f / Parent.rect.height);
            this.rectTransform.anchorMax += Vector2.Scale(this.rectTransform.offsetMax, ParentSize);
            this.rectTransform.anchorMin += Vector2.Scale(this.rectTransform.offsetMin, ParentSize);
            this.rectTransform.offsetMin = Vector2.zero;
            this.rectTransform.offsetMax = Vector2.zero;
        }

        public void CustomAllAnchor()
        {
            var ChildsNormalButton = this.transform.GetComponentsInChildren<ImageUI>();
            foreach (var button in ChildsNormalButton)
            {
                button.CustomAnchor();
            }
        }
