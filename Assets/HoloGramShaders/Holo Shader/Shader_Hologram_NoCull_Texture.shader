...
Shader "Shader Forge/Shader_Hologram_NoCull_Texture" {
	Properties{
		...
		SubShader{
			...
			Pass {
				...
				float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
					...
					float3 emissive = (((_Brightness * (lerp(_Diffuse_var.rgb,dot(_Diffuse_var.rgb,float3(0.3,0.59,0.11)),_DesatAmount) * (_Diffuse_var.rgb * _ColorIntensity) * _Diffuse_var.rgb) * ((1.0 - saturate((sceneZ - partZ) / _IntersectAttenuation)) + _IntersectBrightness + pow(1.0 - max(0,dot(normalDirection, viewDirection)),(1.0 - _FresIntensity)))) + node_5793) * (node_5793 + _ObjectOpacity + lerp(0.0,((node_8683 * 0.5 + 0.5) * _StaticIntensity),_StaticIntensity)));
			...
			Pass {
				...
				float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
					...
					float2 node_8683_rnd = 4.789 * sin(489.123 * (node_8683_skew));
					float node_8683 = frac(node_8683_rnd.x * node_8683_rnd.y * (1 + node_8683_skew.x)); // Static effect
					o.Emission = (((_Brightness * (lerp(_Diffuse_var.rgb,dot(_Diffuse_var.rgb,float3(0.3,0.59,0.11)),_DesatAmount) * (_Diffuse_var.rgb * _ColorIntensity) * _Diffuse_var.rgb) * ((1.0 - saturate((sceneZ - partZ) / _IntersectAttenuation)) + _IntersectBrightness + pow(1.0 - max(0,dot(normalDirection, viewDirection)),(1.0 - _FresIntensity)))) + node_5793) * (node_5793 + _ObjectOpacity + lerp(0.0,((node_8683 * 0.5 + 0.5) * _StaticIntensity),_StaticIntensity)));
